...
class EngagementViewModel(private val repository:EngagementRepository?):ViewModel(){private val mutable=MutableStateFlow(EngagementState());val state=mutable.asStateFlow();fun load(userId:String?){if(userId==null){mutable.value=EngagementState();return};if(userId==mutable.value.userId)return;viewModelScope.launch{mutable.value=EngagementState(true,userId);runCatching{EngagementState(false,userId,repository?.notifications(userId).orEmpty(),repository?.referralCode(userId),repository?.rewards(userId).orEmpty())}.onSuccess{mutable.value=it}.onFailure{mutable.value=EngagementState(false,userId,error="Notifications and referrals could not be loaded.")}}}fun read(item:NotificationItem){val user=mutable.value.userId?:return;viewModelScope.launch{repository?.markRead(item.id,user);mutable.value=mutable.value.copy(notifications=mutable.value.notifications.map{if(it.id==item.id)it.copy(readAt="read")else it})}}}
